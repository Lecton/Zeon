...
import server.database.objects.*;
...
//    }
    ...
    public static Greeting userLogin(Channel ch, String username, String passwordHash) {
        ...
        if (userID != ERROR) {
            ...
            NewUser newUserMessage = MessageBuilder.generateNewUser(getUserProfile(userID));
            for (Channel channel: clients) {
                Client cl = (Client)channel;
                System.err.println(cl.getUserID() + " --> " + userID);
                channel.writeAndFlush(newUserMessage);
            }
            return MessageBuilder.generateGreeting(getUserProfile(userID), true);
        }
   ...
   /*******************The hell is this?********************/
   /* public server.database.objects.User getNewUser(int userID){
        server.database.objects.User u = getUserProfile(userID);
        if(u != null){
            return ;
        }
        return null;
    ...
    */
    ...
    protected static server.database.objects.User getUserProfile(int userID){
        ObjectSet<server.database.objects.User> result = DB.getAllUsers();
        for(server.database.objects.User p: result){
